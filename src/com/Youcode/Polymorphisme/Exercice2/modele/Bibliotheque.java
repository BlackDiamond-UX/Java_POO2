...
//Method ajoutAbonne
    public Abonne ajoutAbonne(int id, String nom, int numAbonnement, Ouvrage ouvrage){
        if (!ID.contains(id)){
            ID.add(id);
            return new Abonne( id, nom, numAbonnement,  ouvrage);
        }else {
            ...
            if (!ID.contains(newID)){
                ID.add(newID);
                return new Abonne( newID, nom, numAbonnement, ouvrage);
            }else {
                return ajoutAbonne(id, nom, numAbonnement, ouvrage);
            }
        }
